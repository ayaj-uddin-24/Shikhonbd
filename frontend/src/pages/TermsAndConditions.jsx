import React from "react";
import { Helmet } from "react-helmet";

const TermsAndConditions = () => {
  return (
    <div className="px-5 sm:px-[5vw] lg:px-[9vw] py-5">
      <Helmet>
        <title>Terms and Conditions</title>
      </Helmet>
      <p className="text-3xl font-bold">শর্তাবলী এবং নিয়মাবলী</p>
      <br />
      <p className="font-bold">শিখনবিডির পিডিএফ ফাইল ব্যবহার নীতিমালা :</p>
      <br />
      <p>
        শিখনবিডি সবার জন্য বিনামূল্যে পিডিএফ নোট, শীট এবং সাজেশন প্রকাশ করে
        থাকে। একজন শিক্ষার্থী, শিক্ষক এবং অভিভাবক হিসেবে আপনি এই রিসোর্সগুলো
        সম্পূর্ণ বিনামূল্যে ব্যবহার করতে পারেন তবে কিছুক্ষেত্রে আমাদের পরিসেবা
        গ্রহণে অর্থ প্রদান করার প্রয়োজন হতে পারে । তবে এগুলো কেবলমাত্র ব্যক্তিগত
        ব্যবহারের জন্য, কোনোভাবেই বাণিজ্যিক কাজের স্বার্থে ব্যবহার করতে পারবেন
        না।
      </p>
      <br />
      <p>
        শিখনবিডি থেকে সংগ্রহ করা রিসোর্সগুলো নাম, লোগো এবং জলছাপ পরিবর্তন করে
        ব্যবহার করা সম্পূর্ণ অনৈতিক এবং এটি কপিরাইট আইনের অধীন। আমরা কোনোভাবেই
        এমন কর্মকাণ্ডের অনুমতি দেই না। তাই শিখনবিডি হতে প্রাপ্ত পিডিএফ নোট, শীট
        এবং সাজেশন অপরিবর্তিত অবস্থায় ব্যবহার একান্ত কাম্য।
      </p>
      <br />
      <p className="font-bold">
        শিখনবিডি ব্যবহার করুন নিম্নলিখিত শর্ত সাপেক্ষে :
      </p>
      <br />
      <ol className="pl-12">
        <li>
          {" "}
          শিখনবিডি'তে প্রকাশ করা নিবন্ধগুলো আপনার সাধারণ তথ্য এবং কেবল
          ব্যবহারের জন্য। এগুলো আমরা কোন প্রকার নোটিশ ছাড়াই পরিবর্তন করার
          অধিকার রাখি।{" "}
        </li>
        <li>
          এই ওয়েবসাইটে আপনার ব্যক্তিগত কোন তথ্য বা উপাত্ত সম্পূর্ণরূপে নিজ
          দায়িত্বে ব্যবহার করুন।{" "}
        </li>
        <li>
          শিখনবিডিতে এমন কিছু উপাদান রয়েছে যা আমাদের মালিকানাধীন। যেমন নিবন্ধ,
          ছবি, ভিডিও ইত্যাদি। কপিরাইট আইন অনুসারে শিখনবিডি থেকে এই সকল
          উপাদানগুলো অন্য কোথাও ব্যবহার থেকে বিরত থাকুন। ইহা দণ্ডনীয় অপরাধ।
        </li>
        <li>
          {" "}
          বিভিন্ন সময়ে শিখনবিডির বিভিন্ন নিবন্ধে অন্যান্য ওয়েবসাইটের লিঙ্ক
          অন্তর্ভুক্ত থাকতে পারে। এই লিঙ্কগুলো বিস্তারিত তথ্যের জন্য এবং আপনার
          সুবিধার জন্য সরবরাহ করা হয়। তবে তারমানে এই নয় যে, আমরা উক্ত
          ওয়েবসাইটের সকল কর্মকাণ্ড সমর্থন করি। লিঙ্কযুক্ত ওয়েবসাইট (গুলো) এর
          সামগ্রীর জন্য আমাদের কোনও দায়বদ্ধতা নই।
        </li>
      </ol>
      <br />
      <p>
        সর্বপরি শিখনবিডির সকল তথ্যবলী আপনার ব্যক্তিগত কাজের জন্য ব্যবহার করুন।
        প্রাতিষ্ঠানিক কাজে কিংবা বাণিজ্যিক উদ্দ্যেশ্যে শিখনবিডির উপকরণগুলো
        পরিবর্তন কিংবা পরিমার্জন করা থেকে বিরত থাকুন। অর্থাৎ কোন প্রকার অনুমতি
        ব্যতীত এই ওয়েবসাইটের কোনো লেখা, ছবি, অডিও, ভিডিও ব্যবহার বেআইনি।
      </p>
      <br />
      <p className="font-bold">আপনি অবশ্যই যা থেকে বিরত থাকবেন :</p>
      <br />
      <ol className="pl-12">
        <li> শিখনবিডির নিবন্ধগুলো অন্য কোনো ওয়েবসাইটে প্রকাশ করবেন না।</li>
        <li>
          শিখনবিডির ছবি বা ভিডিওগুলো অন্য কোনো ওয়েবসাইটে প্রকাশ করবেন না।
        </li>
        <li>
          শিখনবিডির সকল উপকরণ বাণিজ্যিক উদ্দেশ্যে ব্যবহার থেকে বিরত থাকবেন।
        </li>
      </ol>

      <br />
      <br />

      <p className="text-3xl font-bold">Terms and Conditions</p>
      <br />
      <p className="font-bold">Usage Policy for ShikhonBD's PDF Files :</p>
      <br />
      <p>
        Usage Policy for ShikhonBD's PDF Files: ShikhonBD provides free PDF
        notes, sheets, and suggestions for everyone. As a student, teacher, or
        guardian, you can use these resources entirely free of charge. However,
        certain services may require a payment. These resources are strictly for
        personal use and cannot be used for commercial purposes under any
        circumstances.
      </p>
      <br />
      <p>
        It is unethical and a violation of copyright law to modify the name,
        logo, or watermark of resources collected from ShikhonBD. We do not
        permit such actions. Therefore, it is essential to use ShikhonBD's PDF
        notes, sheets, and suggestions in their original, unaltered state.
      </p>
      <br />
      <p className="font-bold">
        Use ShikhonBD under the following conditions :{" "}
      </p>
      <br />
      <ol className="pl-12">
        <li>
          The articles published on ShikhonBD are for general information and
          personal use only. We reserve the right to modify these materials
          without prior notice.
        </li>
        <li>
          Any personal data or information used on this website is entirely at
          your own responsibility.
        </li>
        <li>
          ShikhonBD owns materials such as articles, images, and videos. As per
          copyright law, refrain from using these materials elsewhere without
          permission, as it is a punishable offense.
        </li>
        <li>
          At times, articles on ShikhonBD may include links to other websites
          for detailed information or user convenience. However, this does not
          imply that we endorse all activities on those websites. We are not
          liable for the content of linked websites.
        </li>
      </ol>
      <br />
      <p>
        In summary, use all information and materials from ShikhonBD strictly
        for personal purposes. Refrain from modifying or altering these
        materials for institutional or commercial purposes. Using any text,
        images, audio, or video from this website without permission is illegal.{" "}
      </p>
      <br />
      <p className="font-bold">Prohibited Activities : </p>
      <br />
      <ol className="pl-12">
        <li> Do not publish ShikhonBD's articles on any other website.</li>
        <li>Do not share ShikhonBD's images or videos on other platforms.</li>
        <li>
          Refrain from using ShikhonBD's materials for commercial purposes.{" "}
        </li>
      </ol>
    </div>
  );
};

export default TermsAndConditions;
